...
public class DefaultServlet extends HttpServlet {
	...
	public void init() throws ServletException {
		try {
			...
			if (_context != null) {
				...
				if (file.mkdirs()) {
					LOGGER.info("Temporary path created. Path:" + _root_path);
				} else {
					LOGGER.warn("Unable to create temporary path. Path:" + _root_path);
				}
			...
			new Thread(() -> {
                try {
                    ControllerUtil.scan();
                    LOGGER.info("Successfully Scanned Controllers. Controller Count: " + ControllerUtil.PATH_MAP.size());
                } catch (Exception e) {
                    LOGGER.error("Unable to Scan Controllers.", e);
                }
            }).start();
			...
			new Thread(() -> {
                MessageReader.read();
                LOGGER.info("Successfully loaded the `messsages`");
            }).start();

			ConnectionFactory.init();
		} catch (UnknownHostException e) {
		    LOGGER.error("Failed to initialize the DefaultServlet ", e);
		}
	}
	...
	public void destroy() {
		...
		try {
			...
			LOGGER.error("Unable to remove messages", e);
		}
		super.destroy();
	}
	...
	private void _doProcess(HttpServletRequest request, HttpServletResponse response, ResolveType resolveType)
		...
		if (ControllerUtil.PATH_MAP.containsKey(path)) {
			...
			if (ConnectionFactory.isConfigured()) {
				...
				case CONFIG:
					if (AppConstants.PATH_LOGIN.equals(path)) {
			...
			FrontController<Bean> frontController = new FrontController<>();
				...
				if (session.getAttribute(Constants.SESSION_CONNECT) != null) {
					if (ConnectionFactory.isConfigured()) {
						...
						case CONFIG:
							...
							response.sendRedirect(request.getContextPath() + AppConstants.PATH_CONNECTION_ERROR);
							break;
						case HALF_CONFIG:
						default:
							session.invalidate();
			...
			response.sendRedirect(request.getContextPath() + AppConstants.PATH_ERROR);
		}
		RequestAdaptor.REQUEST_MAP.remove(Thread.currentThread().getId());
	...
	private void _setNewAdd(HttpServletRequest request, final EncodeHelper encodeObj) {
		...
		try {
		    ...
		    LOGGER.error("Unable to add attribute to httpServletRequest", e);
		}
    }
